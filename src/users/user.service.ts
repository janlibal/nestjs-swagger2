...
export class UserService {
  ...
  async register(dto: AuthRegisterLoginDto): Promise<void> {
    ...
    if (user.role) {
      const roleObject = Object.values(RoleEnum).map(String).includes(String(user.role))
      if (!roleObject) {
    ...
    if (user.status) {
      const statusObject = Object.values(StatusEnum).map(String).includes(String(user.status))
      if (!statusObject) {
